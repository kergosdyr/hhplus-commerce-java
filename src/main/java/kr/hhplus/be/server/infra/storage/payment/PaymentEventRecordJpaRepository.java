...
public interface PaymentEventRecordJpaRepository extends JpaRepository<PaymentSuccessEventRecord, Long> {
	...
	@Query("select e from PaymentSuccessEventRecord e where e.status = kr.hhplus.be.server.enums.PaymentEventRecordStatus.NOT_SEND order by e.orderCreatedAt DESC")
